...
export const developmentController = {
  deleteDevelopment: {
    ...
    handler: async (request, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send({ message: "Internal Server Error", success: 0 });
      }
    },
  },
      ...
      try {
        ...
        if (loggedin === 1) {
          ...
          if (rows1.length > 0 || rows2.length > 0) {
            ...
            return reply
              .status(404)
              .send({ message: "development not found", success: 0 });
          }
        } else {
          if (rows1.length > 0) {
    ...
    handler: async (request, reply) => {
      ...
      rows.forEach(async (row) => {
        ...
        if (result.length === 0) {
          ...
          const { MAX_DEVELOPMENT, MIN_AGE_MONTH, MAX_AGE_MONTH } =
            developmentResult[0];
        ...
        const [rows1] = await request.server.mysql.execute(finalQuery, [
          minMonth,
          maxMonth,
          minMonth,
          maxMonth,
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        if (childbirth !== childcorrectedbirth) {
          ...
          const correcteddiff = Math.ceil(
            (date2 - date1) / (1000 * 60 * 60 * 24 * 7)
          ); // Difference in weeks
  ...
  typeResultDevelopment: {
    ...
    handler: async (request, reply) => {
      try {
        const [rows1] = await request.server.mysql.execute(
          "SELECT * FROM GL_DEVELOPMENT_TYPE"
        );
        const [rows2] = await request.server.mysql.execute(
          "SELECT * FROM GL_DEVELOPMENT_TYPE_DSPM_DAIM"
        );
        if (rows1.length > 0 || rows2.length > 0) {
          return reply
            .status(200)
            .send({ success: 1, type: rows1, type_DPM_DAIM: rows2 });
        }
        return reply
          .status(404)
          .send({ success: 0, message: "query not found" });
